...
- **Library search over metadata (actors / director / etc.).** Filename search ships today (`/admin/api/search`, `/api/control/search`). Searching by actor/director/synopsis needs a metadata provider (TMDB) since filenames don't carry that. Pairs naturally with the now-playing info card — both want a per-file TMDB lookup cached to disk (keyed by parsed title+year). Build the metadata cache once, drive both the card and a metadata-aware search index off it.

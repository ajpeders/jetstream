...
- **Angular migration** — replacing the hand-written page scripts panel by panel (see ARCHITECTURE → Angular islands). Done: admin data panels, viewer queue/requests/library panels, the home front door (`src/public-app`, 2026-09-07). Remaining, largest first: `library.html` (entirely inline, ~1250 lines — browse, search, VOD player, subtitles, continue-watching; also the file the `ui/daisyui-conversion` branch rewrote, so port from one or the other, not both), `admin.html` file browser + playback/seek/subtitle controls + invites (~1080 lines), `viewer.html` player + reactions (~1090 lines), the three Svelte islands (AdminQueue, AdminRecent, ViewerChat — porting them lets Vite and Svelte be dropped), then `login.html` into the public bundle and `hub.html`.

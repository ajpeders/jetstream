...
When a viewer's `/hls` request reappears (`_viewer_count() > 0`), the watcher materializes real playback with `_start_stream(current_source, start_seconds=<virtual position>)` — a linear-TV drop-in at exactly where the simulation would be. A manual Pause clears `detached` (a real pause, not a simulation). `is_live` sources are never simulated (no duration to advance against). Cost: a ~1-3 s cold start when a viewer connects instead of sitting on a warmed live edge. `0` disables and restores always-transcoding behavior.

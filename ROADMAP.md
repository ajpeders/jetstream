...
- **Tear down `jetstream-dev`.** Once NVENC is verified on prod, remove the dev sister-service: the `jetstream-dev` + `nginx-jetstream-dev` compose blocks, the `jetstream-dev-internal` network, the `jetstream-dev-hls` tmpfs volume, the `state/jetstream-dev/` dir, and the `JETSTREAM_DEV_*` env knobs (`JETSTREAM_DEV_USE_VAAPI_DECODE`, `JETSTREAM_DEV_TARGET_HEIGHT`, `JETSTREAM_DEV_HLS_SEG_TIME`, `JETSTREAM_DEV_HLS_LIST_SIZE`, `JETSTREAM_DEV_USE_NVENC`). It was useful for iterating on HW-codec work in isolation; with that landed it's just RAM + cognitive overhead.
